						...
						<% if $ParentID == 0 %>
							<p>
								<a href="$Link" class="permaLink">
									<span class="typcn typcn-link-outline"></span>&nbsp;<span class="title">Permalink</span>
								</a>
							</p>
						<% end_if %>
					</div>
						...
						<% if $ParentID == 0 || $Top.Options.Threaded %>
							...
							<div class="reply">
								<form id="replyTo$ID" method="POST" action="" class="replyForm <% if not $Top.Options.ShowReply %>visually-hidden<% end_if %>">
									...
									<div class="Actions">
										<input class="button small" type="submit" value="Reply" name="action_savepost" />
										<button class="no-style toggle-reply small">
											<span class="typcn typcn-cancel-outline text-large"></span>&nbsp;Cancel
										</button>
									</div>
								</form>
								<button class="toggle-reply small secondary outline round tight">
